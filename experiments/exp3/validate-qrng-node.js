...
const ENDPOINT = 'http://localhost:8888/.netlify/functions/qrng-validate'; // Validation-specific endpoint with longer timeouts

// Fetch QRNG bits from local endpoint
async function fetchQRNGBits(nBits) {
  const nBytes = Math.ceil(nBits / 8);
  const response = await fetch(`${ENDPOINT}?n=${nBytes}`);

  const data = await response.json();

  if (!response.ok) {
    console.error('\n❌ Server error:', data);
    throw new Error(`HTTP ${response.status}: ${data.error || response.statusText} - ${data.detail || ''}`);
  }
...
async function validateQRNG() {
  ...
  console.log(`\nMake sure your dev server is running (npm start)\n`);
  ...
  try {
    ...
    console.error('\nMake sure:');
    console.error('  1. Your dev server is running (npm start)');
    console.error('  2. The endpoint is accessible at', ENDPOINT);
    process.exit(1);
  }
}
